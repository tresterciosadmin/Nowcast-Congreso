...
Valle explicitó algo que estaba a medias en el plan: las páginas que siguen al Congreso ya muestran el consolidado de votos a favor y en contra de cada legislador. Nuestro diferencial es cruzar eso con el catálogo de temas que armamos (las taxonomías): poder decir "este diputado aprueba casi todo en materia laboral, pero rechaza sistemáticamente lo tributario". Todavía no se puede calcular porque faltan dos piezas que ya están en marcha: que el agente lector de PDFs termine de etiquetar los proyectos por tema, y conectar cada votación con su proyecto de ley. Cuando estén, el Excel de legisladores suma una hoja "PorTema" con ese desagregado. Quedó anotado en el plan como pieza central, no como detalle.
## Avance: ya sabemos cómo tapar el agujero del Senado (2015–2023)
A la base le faltaba un tramo grande del Senado: de 2015 a 2023 no teníamos ningún voto. La buena noticia es que el propio sitio del Senado publica, para cada votación desde hace décadas, una página con el voto de cada senador —y encima dice de qué bloque y provincia es cada uno, un dato que otras fuentes no traen. Armamos un programa que recorre esas páginas año por año y guarda todo en el formato de nuestra base. Es como mandar a un pasante incansable a fotocopiar acta por acta, solo que tarda 20 minutos y no se queja. Ya lo probamos "en seco" (con páginas de práctica, 22 pruebas pasaron); falta la corrida real desde una compu con internet, porque la mía no puede entrar a las webs del Congreso. De paso, cuando corra, ese mismo dato de bloque nos va a servir para arreglar los votos del Senado 2024–2025 que teníamos "sin bloque". También miramos dos atajos: un repo privado con datos de Andy Tow (no pudimos entrar, y su Senado llega hasta 2014) y un dataset de la comunidad que llega hasta 2019 — ese lo vamos a usar como doble control de calidad, no como fuente.

## Cierre: el agujero del Senado (2015–2023) quedó tapado
Corrió el programa y trajo todo: 749 votaciones con los ~54 mil votos de esos nueve años, cada una con la cámara completa (incluidos los ausentes, que nos sirven para estudiar el presentismo). Para estar seguros de que no trajimos basura, comparamos voto por voto contra un dataset independiente que otra persona había armado años atrás del mismo sitio: en casi 44 mil votos comparables, cero diferencias. También encontramos y arreglamos dos defectos en el camino (el "resultado" de cada votación se leía mal, y una columna de más iba a romper la base unificada). Queda una trampa anotada en rojo: el sitio del Senado dice de qué bloque es cada senador HOY, no de qué bloque era cuando votó — así que ese dato hay que reconstruirlo aparte antes de usarlo para medir disciplina. Ese es el próximo paso.

## Avance: le devolvimos a cada voto del Senado el bloque de SU época
El sitio del Senado tenía una trampa: decía de qué bloque es cada senador HOY, no cuando votó (aparecía gente votando por bloques que todavía no existían). Lo arreglamos reconstruyendo un "padrón histórico": para 2017-2023 usamos las listas por período de Wikipedia (decisión de Franco: fuente comunitaria + validación propia, merece ADR), y para 2015-2017 —que Wikipedia no cubre— curamos senador por senador usando el dato real de la semilla de Andy Tow (hasta 2014) y razonando los casos nuevos. Resultado: el 97% de los ~54 mil votos ya tiene el bloque correcto de su época, y los controles anti-anacronismo pasaron de ~4.300 avisos a ~200.

**Atención equipo — hace falta revisión humana:** en `datos/senado/data/padron_manual_2015_2017.csv` hay filas marcadas `REVISAR` en la nota: (1) once senadores cuyo bloque 2015-2017 inferimos (casi todos al FpV-PJ, porque las etiquetas post-ruptura de dic-2017 no existían antes); (2) el sub-bloque exacto dentro del Frente de Todos de los cuatro senadores que en feb-2023 armaron Unidad Federal (les pusimos Frente Nacional y Popular; Franco ya validó Caserio/Perotti y Catalfamo/Espínola). Ese CSV es editable a mano y lo curado ahí le gana al padrón automático. Queda pendiente cerrar ~1.500 votos sin cobertura (variantes de nombres y reemplazos que el matching todavía no une).

## Cierre de jornada: el Senado 2015-2023 quedó completo y con los bloques de época al 100%
Terminamos lo que faltaba: los últimos ~1.500 votos sin bloque eran senadores que Wikipedia se olvidó de listar (¡el anexo 2021-2023 omite a varios que estaban sentados y votando, lo confirmamos contra las actas oficiales en PDF!), una senadora que cambia de nombre según la fuente (Ledesma Abdala "de Zamora"), y una perlita: un senador correntino que aparecía en un bloque "Frente de Todos"... de 2015. No era un error: era un partido de Corrientes que se llamaba igual que la coalición nacional que nació cuatro años después. Lo renombramos para que el sistema no los confunda. Resultado final: los 53.910 votos del Senado 2015-2023 tienen el bloque que correspondía el día de la votación, con cero anacronismos. El padrón curado a mano quedó guardado en el repo como fuente de verdad (con las filas a revisar marcadas para el equipo). Próximo paso natural: enchufar esta fuente a la reconstrucción de la base canónica y volver a medir la disciplina del Senado, ahora sí con bloques verdaderos.
