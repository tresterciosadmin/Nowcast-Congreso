...
| **casos/ (Parte B, PENDIENTE)** | — | — | Unificar los informes bicamerales hacia `proyectar_postura` condicionado por origen fino (hoy usan `proyectar_lineas_alineacion`, que promedia todas las leyes). Proximo incremento; supeditado a la regen + re-medicion con ALIADOS. |
