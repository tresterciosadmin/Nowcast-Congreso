...
Senado: el padrón trae 'partido' (no bloque) y disperso -> bloque queda 'SIN BLOQUE'
(limitación conocida; lo resuelve datos/senado).
"""
...
def main():
    ...
    # --- Senado (bloque no disponible en el detalle: SIN BLOQUE, flag para datos/senado) ---
    for a in _get("/senado/actas/"):
        fecha=str(a.get("fecha",""))[:10]; aid=f"argentinadatos:senado:{a.get('actaId')}"
        for v in (a.get("votos") or []):
            votos_rows.append(dict(schema_version=SV,acta_id=aid,legislador_id=None,
                legislador_nombre=str(v.get("nombre","")).strip(),bloque="SIN BLOQUE",
