...
- **Salidas** en `data/`:
  ...
  - `votaciones_<gobierno>.xlsx` — un Excel por gobierno, hojas **Metodologia / Actas / Votos**. Actas incluye `margen_votos` (afirmativos − umbral, con signo) para filtrar con cualquier vara sin recalcular. Se separan por gobierno porque el millón largo de votos no entra en una hoja de Excel. **SÍ se versionan, y no es transitorio: son EL ENTREGABLE** para que el equipo vea las votaciones sin correr nada (decisión de Franco, 2026-09-08). Son 49,6 MB, el objeto más pesado del repo después de `proyectos.db`, y ese costo está aceptado a cambio de que cualquiera del equipo los abra con doble clic. Ningún código los nombra —los escribe `export_base.py:265` con `f"votaciones_{g}.xlsx"`— así que todo diagnóstico de archivos huérfanos los va a marcar: no lo son.
