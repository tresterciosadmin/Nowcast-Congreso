...
def backtest(canon: Path, disc: Path, n_sims: int, max_actas: int | None,
             reparto_desvio: float = 0.5, seed: int = 0) -> dict:
    ...
    if max_actas:
        ids = ids[:max_actas]
    lineas_all = _linea_bloque_por_acta(votos[votos["acta_id"].isin(ids)])
    lineas_by_acta = {k: g for k, g in lineas_all.groupby("acta_id")}
    votos_by_acta = {k: g for k, g in votos[votos["acta_id"].isin(ids)].groupby("acta_id")}
    ...
    for aid in ids:
        ...
        try:
            r = simular_votacion(lineas, desvios, tipo, cam, n_sims=n_sims,
                                 reparto_desvio=reparto_desvio, seed=seed)
...
def main() -> None:
    ...
    if modo == "backtest":
        ...
        (out / "backtest_agregador.json").write_text(
            json.dumps(r["resumen"], ensure_ascii=False, indent=2))
        r["detalle"].to_csv(out / "backtest_detalle.csv", index=False, encoding="utf-8-sig")
