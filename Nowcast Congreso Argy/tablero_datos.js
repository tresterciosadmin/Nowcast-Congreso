...
const TABLERO = {
  ...
  actualizado_por: "Valle (con Claude) — Sesion 06-ago, CONTROL GENERAL del repo a pedido de Valle: barrido archivo por archivo (1.352 archivos, 632 MB) buscando lo obsoleto, lo contradictorio y lo que se cuenta mal. LO GRUESO: (1) la base canonica figuraba con 834.749 votos en el tablero y ~781k en el plan cuando en disco tiene 1.016.632 — el salto del 31-jul quedo registrado en la bitacora pero nadie lo propago, que es el modo de falla recurrente del proyecto. (2) Los 2 workflows nuevos NUNCA CORRIERON: quedaron en la subcarpeta y GitHub solo lee .github/workflows de la raiz; el tablero los anunciaba como activos. La evidencia es que datos/padron/outputs/ tiene solo el .gitkeep. (3) URGENTE.md tenia una seccion de resueltos y adentro estaba enterrado un pendiente VIVO: la ingesta del Senado leia el padron viejo, asi que los 6.192 votos de 2026 entraban sin bloque. ARREGLADO en codigo (padron vigente mandate-aware + 8 tests, los 72 senadores resuelven); falta re-correr el pipeline. (4) p_embudo.parquet sigue siendo el del 12-jul, generado con el modelo que tenia el bug del one-hot — el ensemble se apoya en eso. (5) Cinco README de modulo decian PENDIENTE / owner vacante estando en curso, entre ellos datos/canonica, que es la fuente de verdad: un README que invita a reclamar un modulo ocupado es el anticolision fallando al reves.",
